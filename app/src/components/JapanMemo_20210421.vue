...
// import pdfExtract from new PDFExtract().PDFEXTRACT;
export default {
  data() {
    return {
      filesToUpload: [],
  ...
  methods: {
      ...
      if (!this.toInsert.docNo) {
        ...
        Swal.fire({
          ...
          if (result.value) {
            ...
            let url = `${this.japanmemoURL}japan_memo`;
            ...
            axios.post(url, this.toInsert).then(() => {
              Swal.fire({
                icon: "success",
                title: "Success!",
                showConfirmButton: false,
    ...
    openMemo(data) {
      ...
      let url = `${this.japanmemoURL}addCount/memo/${data.id}?category=${data.category}`;
      axios.defaults.headers.common["x-api-key"] =
        "H8Pt5n6cbFk95UiXQjwZ9cwu6mcZqfF2rTw3HeS3";
      axios.post(url).then(() => {
        // console.log(res.data)
      ...
      if (!this.composing) {
        ...
        if (this.searchText) {
          ...
          if (this.selectedCondition == "OR") {
            ...
            url = `${this.japanmemoURL}search_memo/or?search=${search}&except=${this.exceptText}`;
          } else {
            ...
            url = `${this.japanmemoURL}search_memo/and?search=${search}&except=${this.exceptText}`;
          }
      ...
      if (!this.composing) {
        ...
        if (this.searchText) {
          axios.defaults.headers.common["x-api-key"] =
            "H8Pt5n6cbFk95UiXQjwZ9cwu6mcZqfF2rTw3HeS3";
          let url = "";
          url = `${this.japanmemoURL}search_memo/multi?search=${search}&except=${this.exceptText}`;
          axios.get(url).then((res) => {
    ...
    //     userId: this.userInfo.id,
    //   };
    //   let url = `${this.japanmemoURL}getFavorites/memo`;
    ...
    //         let url2 = `${this.japanmemoURL}getDetailsFavorite/memo?id=${this.favoriteList[i].id}&category=${this.favoriteList[i].category}`;
    //         // console.log(url2)
    ...
    loadAllMemo() {
      this.loadingJapanMemo = true;
      this.memoData = [];
      let url = `${this.japanmemoURL}load_japan_memo`;
      axios.defaults.headers.common["x-api-key"] =
        "H8Pt5n6cbFk95UiXQjwZ9cwu6mcZqfF2rTw3HeS3";
      axios.get(url).then((res) => {
        // this.memoData = res.data;
        this.memoData = res.data.map(temp => {
          delete temp.textContent
          return temp
        });
