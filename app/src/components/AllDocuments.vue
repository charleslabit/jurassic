        ...
        <v-row dense no-gutters>
          ...
          <v-col lg="3" md="4" sm="12" cols="12">
            <v-select
              ...
              outlined
              hide-details
              :prepend-icon="computedSortCategoryIcon"
              @click:prepend="sortByCategory()"
              :style="computedRowWidth"
              label="カテゴリー"
              :items="categoryList"
          ...
          <v-col lg="3" md="4" sm="12" cols="12">
            <v-row dense no-gutters :style="computedRowWidth">
              <v-btn
                ...
                outlined
                :style="computedSelectWidth"
                @click="sortByMemoDate()"
              >
                日本メモ更新日
                <v-icon>
                  {{ computedSortMemoButtonIcon }}
                </v-icon>
              </v-btn>
            </v-row>
          </v-col>
    ...
    >
      <v-card v-for="(item, i) in pageData" :key="i" flat>
        ...
        <v-row dense style="cursor:pointer">
          <v-col lg="6" md="12" sm="12" cols="12">
            <div id="id-title" class="ma-2;">
              ...
              <v-icon
                ...
                rounded
                small
                :color="favoriteUsers.includes(item._id) ? `yellow` : `black`"
              >
                mdi-star
              </v-icon>
              ...
              <v-tooltip bottom v-if="item._id">
                <template v-slot:activator="{ on }">
              ...
              <v-tooltip bottom v-else>
                <template v-slot:activator="{ on }">
                  <span v-on="on" @click="openMemo(item)" color="red">
                    {{ item.docNo }} - <span v-html="item.title"></span>
                  </span>
                </template>
                <span v-html="item.title"></span>
              </v-tooltip>
          ...
          <v-col lg="1" md="2" sm="6" cols="4" v-if="item.category">
            <v-chip small dark color="#5c969e" class="mt-2">
              <span id="id-center">
                {{ item.category }}
              </span>
          ...
          <v-col lg="1" md="2" sm="6" cols="4" v-if="item.memoUpdatedDate">
            <v-chip small dark color="#3d7ea6" class="mt-2">
              <span id="id-center">
          ...
          <v-col lg="1" md="2" sm="6" cols="4" v-if="item._id">
          ...
          <v-col lg="1" md="2" sm="6" cols="12" v-if="item.docNo">
            <v-chip small dark color="#26474E" class="mt-2">
              <span id="id-center">
                {{ item.FavoriteCount }}
                <v-icon small>mdi-star</v-icon>
...
export default {
  ...
  computed: {
    ...
    computedAllData() {
      return this.allData
        ...
        .sort((a, b) => {
          if (this.enableSortByMemoDate) {
            const newA = new Date(a.memoUpdatedDate);
            const newB = new Date(b.memoUpdatedDate);
            if (this.sortByMemoDateASC) {
              return newA - newB;
            } else {
              return newB - newA;
  ...
  methods: {
    ...
    sortByProduct() {
      ...
      this.enableSortByFavorite = false;
      this.enableSortByCategory = false;
      this.enableSortByDate = false;
      this.enableSortByMemoDate = false;
    },
    sortByMethod() {
    ...
    sortByCategory() {
      ...
      this.enableSortByCategory = true;
      this.enableSortByProductType = false;
      this.enableSortByMethod = false;
      this.enableSortByCriteria = false;
      this.enableSortByFavorite = false;
    ...
    sortByMemoDate() {
      this.sortByMemoDateASC = !this.sortByMemoDateASC;
      this.enableSortByMemoDate = true;
      this.enableSortByDate = false;
      this.enableSortByCriteria = false;
      this.enableSortByProductType = false;
      this.enableSortByMethod = false;
    ...
    //       console.log(res.data);
    //       if (res.data) {
    //         this.favoriteUsers.push([...res.data]);
    //       }
    //       console.log("FAVORITES", this.favoriteUsers);
    //     });
    //   }
    // },
    ...
    onSearch() {
      ...
      if (this.searchText) {
        ...
        if (this.selectedCondition == "OR") {
          ...
          url2 = `${this.awsURL}search_memo/or?search=${search}&except=${this.exceptText}`;
        } else {
          url = `${this.api}search/get/content/and?search=${search}&except=${this.exceptText}`;
          url2 = `${this.awsURL}search_memo/and?search=${search}&except=${this.exceptText}`;
        }
        axios
          .get(url)
          .then((res) => {
    ...
    onMultipleSearch() {
      ...
      if (this.searchText) {
        ...
        axios
          ...
          .then(() => {
            let url2 = `${this.awsURL}search_memo/multi?search=${search}&except=${this.exceptText}`;
            axios
              .get(url2)
              .then((res) => {
                if (typeof res.data != "string") {
              ...
              .then(() => {
                this.allData.push(...this.memoData);
                this.loading = false;
