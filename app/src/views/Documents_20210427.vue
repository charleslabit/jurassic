      ...
      axios.get(urlDate).then((res) => {
        ...
        if (kind == "Before") {
          if (this.newDocument.personInCharge && scheduleDate) {
            let url = `${this.api}process/create/new_document/${id}?person=${this.newDocument.personInCharge}&date=${scheduleDate}&updatedby=${updatedby}`;
            axios.post(url).then(() => {
          ...
          .filter((rec) => {
            if (rec.personInCharge) {
              if (this.selectedBeforeTheMeeting.value != "All") {
                ...
                ) || rec.personInCharge.toLowerCase() == this.selectedBeforeTheMeeting.text.toLowerCase()
              } else {
                return rec;
              }
            }
