  ...
  <v-container pa-1 ma-1 fluid>
    <v-toolbar dense flat class="mt-1">
      ...
      <v-row no-gutters dense>
        ...
        <v-col v-if="this.userInfo.EmployeeNumber == '0392'">
    ...
    <v-data-table :headers="headers" :items="yearData">
      <template v-if="this.userInfo.EmployeeNumber == '0392'" v-slot:item.actions="{ item }">
        <v-icon
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Before the Meeting Schedule Date", value: "before", divider:true },
        { text: "Meeting Schedule Date", value: "meeting", divider:true },
        { text: "After the Meeting Schedule Date", value: "after", divider:true },
  ...
  methods: {
    saveSchedule() {
      ...
      let url = `${this.api}update/schedules?year=${this.year}`
    ...
    loadAllSchedule(year) {
      this.yearData = []
      let url = `${this.api}get/schedules?year=${year}`;
      axios.get(url).then((res) => {
        ...
        for (let x=0; x<this.yearData.length; x++){
          if(this.yearData[x].before && this.yearData[x].meeting && this.yearData[x].after){
            this.yearData[x+1].enabled = true
            this.yearData[x].enabled = true
          }
          else if (this.yearData[x].before || this.yearData[x].meeting || this.yearData[x].after) {
            this.yearData[x].enabled = true
          }
        }
      });
    ...
    saveYear() {
      let url = `${this.api}add/year/schedule?year=${this.scheduleYear}`;
